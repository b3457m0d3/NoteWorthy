...
public final class DrawingActivity extends Activity implements Runnable, OnPageUpdatedListener, OnReplayCompletedListener {
    ...
    private boolean onSave() {
        ...
        if (this.spdPath == null) {
            ...
            //change this to name it a default name
            //but first check to see if one exists
            //if it does append a number to the end 
        ...
        try {
            ...
            // It is better to save the thumbnail to a separate file because
            // we cannot get the thumbnail from a .spd file without first initializing 
            // PenService (and this takes time).
    ...
    private boolean saveThumbnail(final String thumbnailPath) {
    	...
    	//TODO use the database to link thumbnails to book pages
    	if(this.service.page.exists() && this.service.page.isValid()){
	    	if(this.service.page.thumb.isEmpty()){
	    		this.service.page.setThumb(thumbnailPath);
	    		this.service.page.save();
	    	}
    	}
