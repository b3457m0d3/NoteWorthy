...
import android.lib.pen.demo.R;
import android.lib.sqlite.CursorList;
import android.lib.sqlite.ManyQuery;
import android.lib.sqlite.Query;
import android.lib.sqlite.models.Note;
import android.lib.sqlite.models.NotePageLink;
import android.lib.sqlite.models.Page;
...
public class PenService implements View.OnClickListener, SpenColorPickerListener, SpenSettingEraserLayout.EventListener, SpenPageDoc.HistoryListener, SpenReplayListener, SpenZoomListener, SpenPenChangeListener, SpenEraserChangeListener, SpenPenDetachmentListener, SpenTouchListener {
	...
	public Page             page;
	public NotePageLink     notePageLink;
    ...
    public void onDestroy() {
        if (this.noteDoc != null) {
        	if(this.note.exists() && this.note.isValid()){ 
        		this.note.save();
        	}
    ...
    public void setCurrentPage(final int page) {
        ...
        if (this.noteDoc != null) {
            if (this.currentPage != page) {
                ...
                if(this.note.exists() && this.note.isValid()){
                	this.note.lastEditedIndex = this.currentPage;
                	this.note.save();
                }
            }
        }
    ...
    public void linkPageToNote(long noteId, long pageId){
    	String sql = "SELECT * FROM notePageLinks WHERE note_id = ?";
    	this.notePageLink = Query.one(NotePageLink.class, sql, noteId).get();
    	if(!this.notePageLink.exists()){
    		this.notePageLink = new NotePageLink(noteId,pageId);
    		this.notePageLink.save();
    	} else {
    		this.notePageLink.setPageId(pageId);
    		this.notePageLink.save();
    	}
    }
    ...
    public void appendPage(final int backgroundColor, final String backgroundImagePath, final int backgroundImageMode) {
        if (this.noteDoc != null) {
            ...
            this.pageDoc = this.noteDoc.appendPage();
            this.pageDoc.setBackgroundColor(backgroundColor);

            if (!TextUtils.isEmpty(backgroundImagePath)) {
                this.pageDoc.setBackgroundImage(backgroundImagePath);
            }
            ...
            if (backgroundImageMode == PenService.MODE_CENTER || backgroundImageMode == PenService.MODE_FIT || backgroundImageMode == PenService.MODE_TILE || backgroundImageMode == PenService.MODE_STRETCH) {
                this.pageDoc.setBackgroundImageMode(backgroundImageMode);
            }

            this.pageDoc.clearHistory();
            this.pageDoc.setHistoryListener(this);
            initPageModel();
            updatePageCount();
            linkPageToNote(this.note.getId(),this.page.getId());
        }
    }
    ...
    public void insertPage(final int pageIndex, final int backgroundColor, final String backgroundImagePath, final int backgroundImageMode) {
        if (this.noteDoc != null) {
            this.pageDoc = this.noteDoc.insertPage(pageIndex);
            this.pageDoc.setBackgroundColor(backgroundColor);
    ...
    /**
     * Keep the number of pages current in the database
     */
    public void updatePageCount(){
    	if(this.noteDoc != null && this.note.exists() && this.note.isValid()){
    		this.note.pages = getPageCount();
    		this.note.save();
    	}
    }
    
    public void updatePageIndex(int newIndex){
    	if(this.pageDoc != null && this.page.exists() && this.page.isValid()){
    		this.page.index = newIndex;
    		this.page.save();
    	}
    }
    ...
    public void movePage(final int pageIndex, final int step) {
        if (this.noteDoc != null) {
        	int newIndex = pageIndex + step;
            this.noteDoc.movePageIndex(this.noteDoc.getPage(pageIndex), step);
            updatePageIndex(newIndex);
            //get the rest of the pages and change their indexes in the database
            String sql = "SELECT * FROM notePageLink WHERE note_id = ?";
            Query.many(NotePageLink.class,sql, this.note.getId()).get();
        }
    }
    ...
    public void load(final String path, final boolean writable) throws SpenInvalidPasswordException, SpenUnsupportedTypeException, SpenUnsupportedVersionException, IOException {
        ...
        if (this.noteDoc.getPageCount() > 0) {
            ...
            this.surfaceView.setPageDoc(this.pageDoc, true);
            this.surfaceView.update();
        }
    ...
    public void initModel(){
    	String uuid = this.noteDoc.getId();
    	String sql = "SELECT * FROM notes WHERE uuid = ?";
    	this.note = Query.one(Note.class, sql, uuid).get();
    }
    
    public void initPageModel(){
    	String uuid = this.pageDoc.getId();
    	String sql = "SELECT * FROM pages WHERE uuid = ?";
    	this.page = Query.one(Page.class, sql, uuid).get();
    }
    ...
    private void initCanvas(final int canvasWidth, final int canvasHeight, final int backgroundColor, final String backgroundImagePath, final int backgroundImageMode) {
        ...
        if (this.noteDoc != null) {
            ...
            this.page = new Page();
            if(this.page.exists()){
            	this.page.uuid = this.pageDoc.getId();
            	this.page.index = this.noteDoc.getPageIndexById(this.page.uuid);
            	this.page.save();
            }
